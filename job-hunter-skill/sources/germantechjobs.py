"""GermanTechJobs adapter — scrapes job listings."""
...
SEARCH_URL = "https://germantechjobs.de/jobs"


class GermanTechJobsAdapter(BaseAdapter):
    name = "germantechjobs"
    ...
    async def fetch_all(self, queries: list[str], locations: list[str]) -> list[JobPosting]:
        ...
        for query in search_terms:
            resp = await self._get(SEARCH_URL, params={"q": query})
            if not resp:
                continue

            soup = BeautifulSoup(resp.text, "html.parser")
            cards = soup.select(".job-card, article, [class*='job-listing']")
            ...
            for card in cards:
                try:
                    ...
                    logger.debug("[germantechjobs] Parse error: %s", e)
        ...
        logger.info("[germantechjobs] Fetched %d unique jobs", len(unique))
        return unique
